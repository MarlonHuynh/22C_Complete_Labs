...
/*
    CHANGES MADE IN LAB 6:
        - changed BSTNode* root to public variable (was private) so it can be accessed by derived class MinHeap 
*/
...
BSTNode* BST::insertRecursive(BSTNode* node, Currency* targetDollar) {
    ...
    return node; // If all other cases failed, then returns the node (usually this is the base root node)
}
/*
